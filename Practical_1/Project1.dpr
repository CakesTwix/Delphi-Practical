...
case AMessage of
  ...
  if LParam=hBtnExit then begin PostQuitMessage(0); Exit; end
  ...
  var hWindow : HWnd;
    ...
    if hWindow<>0 then
      begin
        ...
        hBtnExit:=CreateWindow('BUTTON', 'Вихід',WS_CHILD or BS_DEFPUSHBUTTON or WS_TABSTOP, 100,
        10, 90, 30, hWindow, 0, HInstance, nil);

        //Параметры лейблов
        hLabel_1:=CreateWindow('Static','Это первый статический текст',WS_VISIBLE or WS_CHILD or SS_LEFT, 10,10,360,44,hWindow,0,hInstance,nil);
        hLabel_2:=CreateWindow('Static','Это второй статический текст',WS_VISIBLE or WS_CHILD or SS_LEFT, 10,30,360,44,hWindow,0,hInstance,nil);
        hLabel_3:=CreateWindow('Static','Это третий статический текст',WS_VISIBLE or WS_CHILD or SS_LEFT, 10,50,360,44,hWindow,0,hInstance,nil);

        //Параметры поля ввода
        hEdit:=CreateWindowEx(WS_EX_CLIENTEDGE,'Edit','Введите текст', WS_VISIBLE or WS_CHILD or ES_LEFT or ES_AUTOHSCROLL,10,70,360,23,hWindow,0,hInstance,nil);
